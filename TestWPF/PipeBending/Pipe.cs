...
public class Pipe {
	...
	protected void YBCR2XYZR( ) {
		...
		foreach( var ybc in ybcr ) {
			...
			//旋转轴是临时轴
			Trsf axisT = new();
			axisT = new(currentMoveAxis.XDir.Crossed(currentMoveAxis.ZDir).Reversed( ).ToVec(ybc.R));
			...
			tC = new(currentMoveAxis.XDir.ToVec(addedY));
			...
			Trsf tR = new();
			tR = new(new Quat(bendingAxis, ybc.C));
	...
	protected void makeWireFromYBC( ) {
		...
		foreach( var ybc in ybcr ) {
			...
			//Y
			Trsf tY = new();
			tY = new(currentAxis.XDir.ToVec(ybc.Y));
			...
			//B
			Trsf tB = new();
			tB = new(new Quat(currentAxis.XAxis, ybc.B));
			currentAxis = currentAxis.Transformed(tB);
			//bendingAxis
			Trsf tAxis = new();
			tAxis = new(currentAxis.XDir.Crossed(currentAxis.ZDir).Reversed( ).ToVec(ybc.R));
			Ax1 rotationAxis = new (currentAxis.Location, currentAxis.ZDir);
			rotationAxis = rotationAxis.Transformed(tAxis);
			//CR
			lastPnt = (Pnt)currentAxis.Location.Clone( );
			Trsf tC = new();
			tC = new(new Quat(rotationAxis, ybc.C));
